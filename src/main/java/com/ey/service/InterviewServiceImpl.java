...
public class InterviewServiceImpl implements InterviewService {
    ...
    public InterviewServiceImpl(InterviewRepository interviewRepo, ApplicationRepository appRepo, UserRepository userRepo) {
    ...
    public InterviewResponse schedule(Long applicationId, InterviewRequest request) {
        ...
        // ownership check: HR must belong to same company or be job owner
    ...
    public InterviewResponse reschedule(Long interviewId, InterviewRequest request) {
        Interview iv = interviewRepo.findById(interviewId).orElseThrow(() -> new NotFoundException("Interview not found"));
        String email = (String) SecurityContextHolder.getContext().getAuthentication().getName();
        User current = userRepo.findByEmail(email).orElseThrow(() -> new NotFoundException("User not found"));
    ...
    public List<InterviewResponse> getMyInterviews() {
        ...
        if (current.getRole() == Role.HR) {
            ...
            // seeker
            list = interviewRepo.findByApplicationIdIn(
                appRepo.findBySeekerId(current.getId()).stream().map(Application::getId).collect(Collectors.toList())
            );
        }
