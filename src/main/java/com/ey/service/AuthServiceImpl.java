...
public class AuthServiceImpl implements AuthService {
    ...
    private final String resetUrlPrefix; // front-end reset URL prefix (prefix + token)

    // token expiry in minutes (configurable)
    ...
    public SimpleResponse forgotPassword(ForgotPasswordRequest request) {
        ...
        if (opt.isEmpty()) {
            ...
            // But we still return a generic message.
        ...
        // send email (simple plain-text)
        String resetLink = resetUrlPrefix + token;
        SimpleMailMessage message = new SimpleMailMessage();
        message.setTo(user.getEmail());
        message.setSubject("Password reset request");
        message.setText(buildResetEmailText(user.getName(), resetLink));
        // from address will be taken from mail properties (spring.mail.username) if configured
        try {
            mailSender.send(message);
        } catch (Exception ex) {
            // If email sending fails, remove token and return error
