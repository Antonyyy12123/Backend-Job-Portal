...
public class JobServiceImpl implements JobService {
	...
	public List<JobResponse> getJobsForCurrentHr() {
		String email = (String) SecurityContextHolder.getContext().getAuthentication().getName();
		User hr = userRepo.findByEmail(email).orElseThrow(() -> new NotFoundException("HR not found"));
		return jobRepo.findByHrIdOrderByCreatedAtDesc(hr.getId()).stream().map(this::toDto)
				.collect(Collectors.toList());
	}
	...
	private JobResponse toDto(Job j) {
		...
		if (j.getRequirements() != null) {
			r.setRequirements(
					j.getRequirements().stream().map(JobRequirement::getRequirement).collect(Collectors.toList()));
		} else {
