...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiError> handleAccessDenied(AccessDeniedException ex, HttpServletRequest req) {
		ApiError err = new ApiError(HttpStatus.FORBIDDEN.value(), "Access Denied", ex.getMessage(),
				req.getRequestURI());
		return new ResponseEntity<>(err, HttpStatus.FORBIDDEN);
	}
	...
	protected ResponseEntity<Object> handleValidation(MethodArgumentNotValidException ex, WebRequest request) {
		...
		ApiError err = new ApiError(HttpStatus.BAD_REQUEST.value(), "Validation Failed", "Input validation failed",
				req.getRequestURI());
