...
public class InterviewController {
    ...
    public ResponseEntity<?> schedule(@PathVariable Long applicationId, @RequestBody InterviewRequest request) {
        return ResponseEntity.status(201).body(interviewService.schedule(applicationId, request));
    }
 
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('HR')")
    public ResponseEntity<?> reschedule(@PathVariable Long id, @RequestBody InterviewRequest request) {
